 ...
 * </p>
 * 
 * @deprecated The statuses reported by the {@link org.eclipse.emf.transaction.TXEditingDomain}
 *     API in a {@link org.eclipse.emf.transaction.RollbackException} typically
 *     are either {@link org.eclipse.emf.validation.model.IConstraintStatus}es
 *     or multi-statuses of <code>IConstraintStatus</code>es.  The exception
 *     to this rule is when a transaction aborts for some other reason, such as
 *     a run-time exception during commit, in which case the status is of some
 *     other, unprescribed type.
 */
