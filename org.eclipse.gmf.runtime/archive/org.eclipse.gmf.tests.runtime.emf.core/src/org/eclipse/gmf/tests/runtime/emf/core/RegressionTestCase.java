...
//		}
	...
	public void test_idempotentCommands_117131() {
		...
		for (Iterator iter = compound.getCommands().iterator(); iter.hasNext();) {
			...
			if (next instanceof MSLSetCommand) {
				...
				if ((cmd.getFeature() == EcorePackage.eINSTANCE.getENamedElement_Name())
						&& (cmd.getOwner() == eclass)) {
					...
					if ("FinalName".equals(cmd.getValue())) { //$NON-NLS-1$
						if ("NewName".equals(cmd.getOldValue())) { //$NON-NLS-1$
							// this is the "good" command, which changed the name
							setCommand = cmd;
						} else if ("FinalName".equals(cmd.getOldValue())) { //$NON-NLS-1$
							// this is the "bad" command, which does not change the name
							badCommand = cmd;
						}
					} 
				}
	 ...
	 * Tests that we do not record in the current undo interval and MCommands
	 * for sets or unsets that do not change the value of an EAttribute.
	 * This particular test checks for correct handling of EEnums.
	 */
	public void test_idempotentCommands_enums_117131() {
		...
		for (Iterator iter = compound.getCommands().iterator(); iter.hasNext();) {
			...
			if (next instanceof MSLSetCommand) {
				...
				// is this the particular command of interest?
				if ((cmd.getFeature() == EXTLibraryPackage.eINSTANCE.getBook_Category())
						&& (cmd.getOwner() == book)) {
					// this command changes the component visibility
					commandCount++;
					
					if (BookCategory.SCIENCE_FICTION_LITERAL.equals(cmd.getValue())) {
						if (BookCategory.MYSTERY_LITERAL.equals(cmd.getOldValue())) {
							// this is the "good" command, which changed the visibility
							setCommand = cmd;
						} else if (BookCategory.SCIENCE_FICTION_LITERAL.equals(cmd.getOldValue())) {
							// this is the "bad" command, which does not change the visibility
							badCommand = cmd;
						}
					} 
				}
		...
		assertNotNull("Did not find the correct component visibility change", setCommand); //$NON-NLS-1$
		
		// should only have two commands that chang the visibility, not three
		assertEquals("Too many changes of the component visibility", 2, commandCount); //$NON-NLS-1$
	}
